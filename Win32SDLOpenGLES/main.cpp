...
SDLTest_CommonState* CommonCreateState(const char* title, Uint32 flags, const char*icon)
{
	int i;
...
SDL_bool CommonInit(SDLTest_CommonState* state)
{
	...
	if (state->flags & SDL_INIT_VIDEO) {
		...
		if (n == 0) SDL_Log("No built-in video drivers\n");
		else {
			SDL_snprintf(text, sizeof(text), "Built-in video drivers:");
			for (i = 0; i < n; ++i) {
				if (i > 0) SDL_snprintfcat(text, sizeof(text), ",");
...
void SDLTest_CommonQuit(SDLTest_CommonState* state)
{
	...
	if (state->targets) {
		for (int i = 0; i < state->num_windows; ++i)
			if (state->targets[i]) SDL_DestroyTexture(state->targets[i]);
	...
	if (state->flags & SDL_INIT_VIDEO) SDL_VideoQuit();
	if (state->flags & SDL_INIT_AUDIO) SDL_AudioQuit();
	SDL_free(state);
	SDL_Quit();
...
void PrintGLInfo() {
	...
	SDL_Log("\n");
	int value;
	int status = SDL_GL_GetAttribute(SDL_GL_RED_SIZE, &value);
	if (!status)
...
void loop(int&done, Uint32&frames)
{
	++frames;
...
int main(int argc, char** argv) {
	...
	state = CommonCreateState("SDL OpenGLES 3.0", SDL_INIT_EVERYTHING, "Resources/icon.bmp");
	if (!state) return 1;
